...
### 3. 빌드 및 배포 안전 규칙 ⚠️ CRITICAL

⚠️ **최우선 규칙**: 사용자가 명시적으로 "빌드해줘", "빌드 시작" 등의 지시를 하기 **전까지는 절대 빌드를 진행하지 않습니다**.

#### **절대 금지 사항**
❌ **사용자의 명시적 요청 없이 절대 실행 금지**:
```bash
# 빌드 명령어
npm run build
npm run build:ios
...
#### **허용되는 동작**
✅ **사용자의 명시적 요청 시에만 실행**:
- "빌드해줘", "빌드 시작해줘", "EAS 빌드 실행해줘" → ✅ 실행
- "배포해줘", "제출해줘", "submit 해줘" → ✅ 실행
- "업데이트해줘", "eas update 해줘" → ✅ 실행
...
⚠️ **중요**: 사용자가 "빌드해줘"라고 명시적으로 지시하기 전까지 절대 빌드를 시작하지 않습니다.

빌드 명령어 실행 전 다음을 사용자에게 확인:

1. **Git 상태 확인 (필수)**
   - `git status` - 커밋되지 않은 변경사항 확인
   - `git log --oneline -3` - 최근 커밋 확인
   - **로컬 커밋이 원격에 push되었는지 확인** ← 매우 중요!
   - EAS는 **원격 저장소**에서 코드를 가져오므로 로컬 커밋만 있으면 빌드에 반영 안 됨

2. **코드 변경사항 확인**
   - 마지막 빌드 이후 변경된 파일 목록
   - 중요 기능 변경 여부
   - 수정사항이 모두 커밋되고 push되었는지
...
6. **최종 확인 메시지 (사용자에게 표시)**
   ```
   🚀 빌드 실행 전 최종 확인:
   - 플랫폼: [iOS/Android/Both]
   - 프로필: [production/preview]
   - 버전: [현재 버전]
   - Git 상태: [push 완료 여부]
   - 예상 소요 시간: [15-20분]

   ⚠️ 위 내용이 맞으면 "빌드 시작" 이라고 명시적으로 말씀해주세요.
   ⚠️ 그 전까지는 빌드를 시작하지 않습니다.
   ```

#### **예외 상황**
다음 경우에만 빌드 명령어를 자동 제안할 수 있음:
- 사용자가 "다음 단계가 뭐야?" 질문 시 → "빌드가 필요합니다" 제안만
- 빌드 관련 문서 작성 시 → 명령어 예시만 제공
- 오류 해결 방안 제시 시 → "빌드 필요할 수 있음" 언급만
...
### 관리자 서버 작업 디렉토리
**모든 관리자 서버 관련 작업은 다음 디렉토리에서 수행할 것:**
```
C:\Users\cntus\Desktop\tarot-admin-dashboard
```

### 관리자 서버 작업 범위
- **관리자 대시보드 개발** (Next.js 기반)
- **분석 데이터 시각화** 및 관리 기능
- **시스템 모니터링** 및 헬스 체크
- **사용자 피드백 관리** 및 커뮤니티 기능
- **Supabase 관리자 전용 기능** 구현

### 관리자 서버 기술 스택
- **프론트엔드**: Next.js 14 + TypeScript + Tailwind CSS
- **백엔드**: Supabase (관리자 전용 스키마)
- **인증**: Auth0 또는 NextAuth.js
- **포트**: 3001 (개발), 3000 (프로덕션)

### 관리자 서버 작업 시 주의사항
1. **보안 최우선**: 관리자 기능은 철저한 인증/인가 필요
2. **데이터 분리**: 사용자 데이터와 관리자 데이터 완전 분리
3. **로그 관리**: 모든 관리자 액션 로깅 및 감사 추적
4. **성능 모니터링**: 실시간 시스템 상태 모니터링 구현
