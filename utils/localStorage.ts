...
export class LocalStorageManager {
  ...
  static async checkTrialStatus(): Promise<PremiumStatus> {
    // ==================================================================================
    // ⚠️ APPLE REVIEW FIX: 7일 무료 체험 기능 일시 비활성화
    // ==================================================================================
    //
    // 📌 비활성화 이유:
    //    - Apple 앱 심사에서 "인앱 구매를 찾을 수 없음" 거절 사유 발생
    //    - 자동 무료 체험 활성화로 인해 심사팀이 구독 화면을 테스트하지 못함
    //    - 샌드박스 환경에서 즉시 프리미엄 기능 사용 가능 → 구매 플로우 우회
    //
    // 📌 재활성화 방법 (앱 승인 후):
    //    1. 아래 주석 처리된 코드(Line 375-436)의 주석을 제거
    //    2. 현재 return 문(Line 366-371) 삭제
    //    3. console.log 메시지 업데이트
    //    4. 빌드 번호 증가 (app.json의 buildNumber)
    //    5. 업데이트 빌드 제출
    //
    // 📌 영향:
    //    - 신규 사용자: 무료 버전으로 시작 → 구독 구매 필요
    //    - 기존 구독 사용자: 영향 없음 (IAPManager에서 실제 구독 상태 확인)
    //    - 광고: 무료 사용자에게 표시됨 (ad_free = false)
    //
    // 📌 변경 이력:
    //    - 2025-10-26: Apple 심사 대응으로 일시 비활성화 (Build 95)
    //
    // ==================================================================================

    console.log('ℹ️ 무료 체험 기능 비활성화됨 (Apple 앱 심사용)');

    // 무료 사용자 상태로 반환 (구독 구매를 통해서만 프리미엄 활성화)
    return {
      is_premium: false,
      ad_free: false,
      unlimited_storage: false,
